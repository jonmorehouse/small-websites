...
var base_gallery = function (images, container, template) {
	...
	function go_to(id) {
		...
		var next = config.container.children("[data-id=" + id + "]");//this will take care of if we are on the last element or not
	...
	function start() {//this is a recursively running function that controls itself as the animation plays!
		...
		var _return = setTimeout(function() {

			start();
		}, config.interval);

		if (!config.playing) return _return;
